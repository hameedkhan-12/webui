import type { IRegistry, ComponentMeta, IAuraKernel, AIContextDescriptor } from '@repo/shared'

export class ComponentRegistry implements IRegistry<ComponentMeta> {
  readonly namespace = 'components'
  private readonly storage = new Map<string, ComponentMeta>()
  private readonly listeners = new Map<string, Set<(value: ComponentMeta | undefined) => void>>()
  private readonly allListeners = new Set<(key: string, value: ComponentMeta | undefined) => void>()
  private readonly deferredAIPushes: ComponentMeta[] = []

  constructor(private readonly kernel?: IAuraKernel) {}

  private flushAIPushes(): void {
    if (this.kernel && this.kernel.lifecycle === 'READY' && this.deferredAIPushes.length > 0) {
      try {
        const aiRegistry = this.kernel.get<AIContextDescriptor>('ai')
        if (aiRegistry) {
